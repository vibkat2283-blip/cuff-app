...
import { Heart, Share2, LogOut, Plus, Lock, Mail, Activity, Droplet, FileText, Check, User, Scale } from "lucide-react";
...
function HistoryRow({ leftLabel, leftSub, zoneLabel, zoneColor, date, isLast }) {
  return (
    <div className="flex items-center justify-between py-2.5" style={{ borderBottom: isLast ? "none" : `1px solid ${COLORS.border}` }}>
      <div className="flex items-center gap-3.5">
        <div className="relative w-2.5 h-2.5 flex items-center justify-center">
          <div className="absolute w-2.5 h-2.5 rounded-full" style={{ background: zoneColor, opacity: 0.25 }} />
          <div className="w-1.5 h-1.5 rounded-full" style={{ background: zoneColor }} />
        </div>
        <div>
          <div className="text-sm font-semibold" style={{ color: COLORS.ink, fontFamily: "'Space Grotesk', sans-serif" }}>
            {leftLabel}
            {leftSub && <span className="text-xs font-normal ml-1.5" style={{ color: COLORS.inkSoft, fontFamily: "Inter, system-ui, sans-serif" }}>{leftSub}</span>}
          </div>
          <div className="text-xs mt-0.5" style={{ color: COLORS.inkSoft }}>{date}</div>
        </div>
      </div>
      {zoneLabel && (
        <span className="text-xs font-semibold px-2.5 py-1 rounded-full" style={{ background: zoneColor + "14", color: zoneColor }}>{zoneLabel}</span>
      )}
    </div>
  );
}
...
export default function App() {
  ...
  const addWeightReading = async () => {
    ...
    const { data } = await supabase.from("weight_readings").insert({
      patient_id: profile.id, value: parseFloat(weightValue), unit: "lbs",
    }).select();
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: COLORS.bg }}>
      <div className="max-w-2xl mx-auto p-5">
        ...
        {latestWeight && (
          <Card>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-sm font-semibold" style={{ color: COLORS.ink }}>Body weight</span>
            </div>
          ...
          <Card>
            ...
            <div className="flex items-end gap-3">
              <div className="flex-1">
                <label className="text-xs block mb-1.5" style={{ color: COLORS.inkSoft }}>Weight (lbs)</label>
                <input type="number" step="0.1" value={weightValue} onChange={(e) => setWeightValue(e.target.value)} placeholder="165" className="w-full rounded-xl px-3 py-2.5 text-sm outline-none text-center" style={{ background: COLORS.surfaceAlt, border: `1px solid ${COLORS.border}`, color: COLORS.ink }} />
              </div>
        ...
        {bpReadings.length > 0 && (
          <Card>
            <span className="text-sm font-semibold block mb-1" style={{ color: COLORS.ink }}>Blood pressure history</span>
            <div className="flex flex-col">
              {bpReadings.map((r, i, arr) => {
                const z = categorizeBP(r.systolic, r.diastolic);
                return (
                  <HistoryRow
                    key={r.id}
                    leftLabel={`${r.systolic}/${r.diastolic}`}
                    leftSub={`${r.pulse} bpm`}
                    zoneLabel={z.label}
                    zoneColor={z.color}
                    date={`${formatDate(r.created_at)} · ${daysAgoLabel(r.created_at)}`}
        ...
        {sugarReadings.length > 0 && (
          <Card>
            <span className="text-sm font-semibold block mb-3" style={{ color: COLORS.ink }}>Blood sugar history</span>
                      ...
                      <div className="flex flex-col">
                        {entries.map((r, i, arr) => {
                          ...
                          return (
                            <HistoryRow
                              key={r.id}
                              leftLabel={`${r.value}${t.unit === "%" ? "%" : ""}`}
                              leftSub={t.unit !== "%" ? t.unit : ""}
                              zoneLabel={z.label}
                              zoneColor={z.color}
                              date={`${formatDate(r.created_at)} · ${daysAgoLabel(r.created_at)}`}
                              isLast={i === arr.length - 1}
        ...
        {weightReadings.length > 0 && (
          <Card>
            <span className="text-sm font-semibold block mb-1" style={{ color: COLORS.ink }}>Body weight history</span>
            <div className="flex flex-col">
              {weightReadings.map((r, i, arr) => (
                <HistoryRow
                  key={r.id}
                  leftLabel={`${r.value} ${r.unit}`}
                  zoneLabel=""
